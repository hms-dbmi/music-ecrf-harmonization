...
display_venn(x = list(
    compare_mappings_subset %>% filter(!is.na(RXNORM.umls)) %>% select(RXNORM.umls) %>% unlist() , 
    compare_mappings_subset %>% filter(!is.na(RXNORM.manual)) %>%select(RXNORM.manual) %>% unlist() , 
    compare_mappings_subset %>% filter(!is.na(RXNORM.mohamad)) %>%select(RXNORM.mohamad) %>% unlist() 
    ),
    category.names = c("UMLS" , "MANUAL" , "MOHAMAD"),
    output = TRUE ,
          imagetype="png" ,

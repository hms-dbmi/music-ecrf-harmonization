...
Patient_dimension_table <- read.delim("local_ref/example_patient_dimension.txt")
...
```{r}
data_dictionnary <- read.csv("../data/MUSIC_DataDictionary_V3_4Dec20_final version_clean_0.csv")%>%
...
```{r}
zip_table <- read.csv("local_ref/Zip_table.csv")
```
...
###Patient id###
creation of a commun ID on the eCRF table and the patient table to be sure that the data will not be mixed up

```{r}
eCRF_table$Patient_id <- Usefull_Patient_table$PATIENT_NUM
...
If there is a zipcode, it means that the patient is a US resident. So we are looking into the eCRF zipcode column. If it is empty the value of the country will be 0, if not it will be 1
```{r}

for (i in 1:dim(eCRF_table)[1]) {
    if (is.null(eCRF_table$zipcodeus[i])) {
      eCRF_table$country[i] = 0
    }else{
      eCRF_table$country[i] = 1
    }
}
...
double loop to check if there is some match between the eCRF zipcode and the zipcode-US-State database, if a match is found, the eCRF state column is filled with the matching US-State

```{r}

for (i in 1:dim(eCRF_table)[1]) {
for (j in 1:dim(zip_table)[1]) {
  
    if (eCRF_table$zipcodeus[i]==zip_table$zipcode[j]) {
      eCRF_table$usstate[i] <- zip_table$state[j]
  
    }else{
      eCRF_table$usstate[i] <- eCRF_table$usstate[i]
      }
}
}
...
double loop to check if there is some match between the eCRF zipcode and the zipcode-county database, if a match is found, the eCRF county column is filled with the matching US-State
...
for (j in 1:dim(zip_table)[1]) {
    ...
    if (eCRF_table$zipcodeus[i]==zip_table$zipcode[j]) {
      eCRF_table$county[i] <- zip_table$county[j]
  
    }else{
      eCRF_table$county[i] <- eCRF_table$county[i]
      }
}
}
...
write.csv(eCRF_table, "Test_eCRF.csv")
```

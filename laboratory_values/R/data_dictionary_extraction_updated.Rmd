...
checked_vars <- read.csv('../data/numericalLabsToReview.csv', skip=1, header = TRUE) 
```
...
We will include all the variables, not only the yes/no ones. 
```{r}
datadict_labonly %>%
...
These variables have already been filtered for when focusing on values which have passed the checkUnits filter.
```{r}
datadict_labonly %>%
...
Unit variables rely on value variables. Summary of the units values are below:
```{r}
datadict_labonly %>%
  filter(vartype == 'unit') %>%
  group_by(Choices..Calculations..OR.Slider.Labels) %>%
...
For the variables for which there is only one unit option, and th eunit listed on the Value Field matched with the one used (as is the case because we filtered by checkUnits), we will write -88. For the ones with multiple options we will put the number for the specific units, 1 or 2. 

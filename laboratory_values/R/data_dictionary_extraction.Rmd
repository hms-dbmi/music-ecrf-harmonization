...
account for: "I reviewed the CRP and Tryglicerides, that were as “specify units below”
they had multiple options with the categorical format: e.g., ###CRP 1, mg/L | 2, mg/dL | 3, Other ….. triglycerids 1, mg/dL | 2, mmol/L | 3, Other; (edited) 
I just checked that the units we had were one of the options they were giving, but not sure were the specific option should be displayed in the final file"
...
remove_vars <- c(remove_vars,
                 paste0('crp', c('_obtained', '_date', '_value', '_unit')),
                 paste0('triglycerid', c('_obtained', '_date', '_value', '_unit')))
...
# Identify variables left out of phase 1
```{r}
remaining_vars <- data.frame("filter" = c('non-numeric', 'unit-mismatch', 'range-mismatch', 'associated-var-mismatch'),
                             "n_vars_affected" = c(300, 3, 27, 3))


remaining_vars %>% arrange(desc(n_vars_affected))

```

# Phase 2
What are the types of non-numeric vars?
```{r}
datadict_labonly %>%
  #filter(vartype == 'value') %>%
  filter(Text.Validation.Type.OR.Show.Slider.Number != 'number')

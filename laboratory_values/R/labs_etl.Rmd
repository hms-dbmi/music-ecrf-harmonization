...
lab_dictionary <- read.csv('../../laboratory_values/data/eCRF_to_TCH_forReview_copy_unitvalue.csv') %>% filter(!is.na(concept_cd))
ecrf <- read.csv('../../common_ref/MUSIC_DataDictionary_V3_4Dec20_final version_clean_0.csv') %>%
  ...
  mutate(Section.Header = gsub("\n", " ", Section.Header), 
         Section.Header = gsub("  ", " ", Section.Header )) %>%
  tidyr::fill(Section.Header) 

lab_dictionary <- right_join(lab_dictionary, ecrf, by = 'Section.Header') %>%
...
all_labs <- read.csv('../local_ref/texasChildren/misc_lab_observations.csv') %>%
  ...
  left_join(read.csv('../local_ref/texasChildren/misc_patient.csv') %>%
    ...
    #filter(inout_cd %in% c('INPATIENT', 'Other')) %>%
...
finalTable <- left_join( subset_df, lab_dictionary %>% select(-concept_cd), by = c('concept_cd' = 'concept_descripiton'))
...
getLabValue <- function(row) {
  ...
  if (row['vartype'] == 'obtained' ) {
    if( row['variableName'] == 'aptt_ptt_obtained'){
      value = '3' #in TCH PTT is the one estimated
    }else{
      value = '1'
    }
    
  # Variables with unit listed below the valaue field will be filled with ‘-88’
  # except for the following exceptions (TCH specific)
    
  } else if (row['vartype'] == 'unit') {
    if( row['variableName'] == 'wbc_unit'){
      value = '1'
    }else if( row['variableName'] == 'platelets_unit'){
      value = '1'
    #}else if( row['variableName'] == 'total_protein_unit'){
    #  value = 'g/dL'
    }else if( row['variableName'] == 'crp_unit'){
      value = '2'
    }else if( row['variableName'] == 'cd19_abs_unit'){
      value = '/uL'
    }else if (row['variableName'] == 'cd3_cd4_abs_unit'){
      value = '/uL'
    }else if (row['variableName'] == 'cd3_cd8_abs_unit'){
      value = '/uL'
    }else if (row['variableName'] == 'cd3_abs_unit'){
      value = '/uL'
    } else {
      value = '-88'
    }
  ...
  # (NOTE: Dates must be imported here only in M/D/Y format or Y-M-D format, regardless of the specific date format designated for this field.)
    ...
    value = as.character(ymd(row['date']))
...
finalTable <- finalTable %>%
  ...
  # mutate(vartype = gsub("^.*_", "", variableName)) %>% done earlier
...
Create a report for TCH of which patients / values require manual review
```{r}
...
unexpected_values = c('Negative', '<0.010', '1+', '2+', '<0.15')
...
write.csv(out, '../local_ref/lab_output_texas_092023.csv', row.names = FALSE)
...
Write output to test upload using dummy IDs 666001 to 666011
```{r}
#dummy_ID_mapping <- data.frame('original_id' = sample(unique(out$record_id), size = 11, replace = FALSE),
#                               'dummy_id' = c(666001:666011))
# use same dummy ID mapping each time

#load('../local_ref/dummy_ID_mapping.RData')
#out2 <- out %>% 
#  filter(record_id %in% dummy_ID_mapping$original_id) %>%
#  left_join(dummy_ID_mapping, by = c('record_id' = 'original_id')) %>%
#  ungroup() %>%
#  select(-record_id) %>%
#  rename('record_id' = 'dummy_id') %>%
#  select(record_id, everything())
#write.csv(out2, '../local_ref/lab_output_texas_092023_subsetforredcap.csv', row.names = FALSE, na = '')

write.csv(out, '../local_ref/lab_output_texas_092023.csv', row.names = FALSE, na = '')
#write.csv(out, '../local_ref/lab_output_texas_092023_testInpatient_other.csv', row.names = FALSE, na = '')

#save(dummy_ID_mapping, file = '../local_ref/dummy_ID_mapping.RData')

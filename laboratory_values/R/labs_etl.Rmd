...
lab_dictionary <- right_join(lab_dictionary, ecrf, by = c('eCRF.TERM.DESCRIPTION' = 'Section.Header')) 
...
newPatients <- read.csv('../../../TCH MUSIC DATA/misc_patient.csv') %>% pull(MUSIC.Patient.ID)
...
all_labs <- read.csv('../../../TCH MUSIC DATA/misc_observations.csv') %>%
  rename(CONCEPT_CD = Concept.code, 
         START_DATE = Date,
         TVAL_CHAR = Categorical.Value,
         NVAL_NUM = Numeric.Value,
         UNITS_CD = Lab.Units) %>%
  left_join(read.csv('../../../TCH MUSIC DATA/misc_patient.csv')) %>%
  rename(ADMISSIONDATE = Admission.Date,
         DISCHARGE_DATE = Discharge.Date, 
         MUSIC.ID = MUSIC.Patient.ID) %>% 
  filter(Patient.Setting == 'INPATIENT')
...
finalTable <- left_join( subset_df, lab_dictionary, by = c("CONCEPT_CD" = "LOINC.CODE") )
...
# group_by( MUSIC.ID, CONCEPT_CD, period ) %>%
...
out <- finalTableFiltered %>%
  group_by(MUSIC.ID, formName, lab_values_visit, redcap_repeat_instance, variableName) %>%
  summarise(value = first(value)) %>%
  pivot_wider(id_cols = c(MUSIC.ID, formName, lab_values_visit, redcap_repeat_instance), 

...
We add them here with the value for obtained being 0
- troponin I
- ah50
- ch50
```{r}
...
value_vars <- grep('_value$', colnames(test_out))
unexpected_values = c('SEE IL12', 'See Image', 'Not Reported', 'Footnote', 'SEE IL17')

# for each row in out
for (r in c(1:nrow(test_out))){
  
  record_id <- test_out$record_id[r]
  visit <- test_out$lab_values_visit[r]

  # for each value variable
  for (c in value_vars) {
    
    variable <- colnames(test_out)[c]

    # if value is in our list of weird things
    if (test_out[r,c] %in% unexpected_values) {
    
      # return list of: record, variable, visit, value
      ret_val <- (c('record_id' = record_id,
               'variable' = variable,
               'visit' = visit,
               'value' = test_out[r,c]))
      
      flagged <- rbind(flagged, ret_val)

...
```{r}
all_labs <- read.delim('../local_ref/ag440_music_labs_nov21.dsv')
subset_df <- all_labs %>%
...
```{r}
misc_mapping <- read.csv("../../medications_before_and_after/local_ref/music_id_mapping.csv")
visit_dates  <- read.csv("../local_ref/MUSIC_DATA_2021-11-23_0940.csv")
...
```{r}
# WRITE SAME CODE TO RE-PLACE THE PREVIOUS DEFINE_TIME_WINDOW FUNCTION
# ASUMING THAT INSTEAD OF ESTIMATING THE TIME WINDOWS WE WILL ONLY EXTRACT THE INFORMATION 
# FOR THOSE VISIT FOR WHAT WE HAVE A FOLLOW-UP DATE

### Issue here is that the dates does not have to match necessarily
### This function allow us to be more or less stric on the days of difference allowed between what is reported in the eCRF and
extractClosestDate <- function( dateVal, dateList, maxDiffAllowed ){
  if( is.na( dateVal )){
    return( NA )
  }else if( dateVal %in% dateList ){
    return( dateVal )
  }else{
    closestDay <- dateList[ which( abs( dateList - dateVal ) == min( abs( dateList - dateVal )))]
    daysDiff <- abs(closestDay - dateVal)
    if( daysDiff <= maxDiffAllowed ){
      return( closestDay )
    }else{
      return( NA )
    }
...
finalTable <- finalTable %>% 
  ...
  mutate(two_weeks_ehr = extractClosestDate(dateVal = unique( ymd( two_weeks )), 
                                            dateList = unique( ymd( date )), 
                                            maxDiffAllowed = daysAllowed), 
         six_weeks_ehr = extractClosestDate(dateVal = unique( ymd( six_weeks )), 
                                            dateList = unique( ymd( date )), 
                                            maxDiffAllowed = daysAllowed), 
         six_months_ehr = extractClosestDate(dateVal = unique( ymd( six_months )), 
                                            dateList = unique( ymd( date )), 
                                            maxDiffAllowed = daysAllowed),
         closes_to_discharge = extractClosestDate( dateVal = unique(ymd( DISCHARGE_DATE )), 
                                              dateList = unique( ymd( date )),
                                              maxDiffAllowed = daysAllowed))


# we need to create a new column: lab_values_visit with values:
### Value = 1 --> the value closest to hospital admission  (no further than 48 hours before or after)
### Value = 2 --> the value closest to discharge (3 days or less before discharge)
### Value = 3 --> the value closest to the 2 weeks post discharge (after discharge until less than three weeks)
### Value = 4 --> the value closest to the 6 weeks post discharge (3 weeks to less then 9 weeks)
### Value = 6 --> the value closest to the 6 months post discharge (9 weeks to less than one year)
####In all cases when we have multiple values for the same lab for the same day we take the closest to the date

getLabValueVisits <- function(row) {
  
  # for obtained variables, if the entry is present, the 'value' is automatically 1
  if (row['date'] == row['ADMISSION_DATE']) {
    value = 1
  }else if (!is.na( row['closes_to_discharge'] ) & row['date'] == row['closes_to_discharge']) {
    value = 2
    }
  else if (!is.na( row['two_weeks_ehr'] ) & row['date'] == row['two_weeks_ehr']) {
    value = 3
  }
  else if (!is.na( row['six_weeks_ehr'] ) & row['date'] == row['six_weeks_ehr']) {
    value = 4
  }
  else if (!is.na( row['six_months_ehr'] ) & row['date'] == row['six_months_ehr']) {
    value = 6
  }else{
    value = NA
  }
...
finalTable$lab_values_visit <- apply(finalTable, 1, FUN = getLabValueVisits)
  
finalTableFiltered <- finalTable %>%
  filter( ! is.na (lab_values_visit )) %>%
  group_by( PATIENT_NUM, CONCEPT_CD, date )%>%
  slice_min( order_by = time) %>%
  unique()

# we check if we have really remove any potential duplicate (same labs measured multiple times during the same day)
check <- finalTableFiltered %>% 
  filter( vartype == "obtained" ) %>%
  group_by( PATIENT_NUM, CONCEPT_CD, date ) %>%
  count( )

# there are still 10 cases, for 2 patients, in which the day and time is the same but there are multiple distinct values
# what should we do in this cases? take one randomly?
### check with Simnran, I think in the code just the first one would be keept 
#check this example at the end jj[ jj$PATIENT_NUM == "37336854" & jj$CONCEPT_CD == "LAB:2700655" & jj$date == "2020-06-25", ]
```

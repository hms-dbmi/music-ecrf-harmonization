...
out <- out %>% 
  mutate(troponin_l_obtained  = 0,
         ah50_obtained  = ifelse(compl_obtained == 1, yes = 0, else = ''), 
         ch50_obtained = ifelse(compl_obtained == 1, yes = 0, else = ''),
...
write.csv(out %>% filter(record_id %in% c(120002, 120112)), '../local_ref/TEST_redcap_output_laboratory_values_updated.csv', row.names = FALSE)

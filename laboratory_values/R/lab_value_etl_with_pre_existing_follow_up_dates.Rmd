...
finalTable <- finalTable %>%
  ...
  distinct %>%
  # filter out labs where reported value is "Not Reported"
  filter(TVAL_CHAR != "Not Reported")
...
write.csv(out %>% filter(record_id %in% c(120002, 120122, 120006)), '../local_ref/TEST_redcap_output_laboratory_values_updated.csv', row.names = FALSE)

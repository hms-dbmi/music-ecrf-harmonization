...
visit_dates  <- read.csv("../local_ref/MUSIC_DATA_2022-03-14_1835.csv")
...
Here we specify for which patients we want to run the pipeline for, the previous patients that we entered data for.
...
```{r}
 ...
 previous_patients <- music_patients_admission_after_nov21 %>%
...
Save the final table to an RData file to generate later the excel sheet for the data that needs to be overwrite. 
```{r}
...
out <- left_join(out, misc_mapping %>% select(PATIENT_NUM = PAT_NUM, MUSIC_ID))
...
write.csv(out, '../local_ref/redcap_output_laboratory_values_all_updated_feb22.csv', row.names = FALSE)
write.csv(out %>% filter(record_id %in% c(120002, 120122, 120018, 120006)), '../local_ref/TEST_redcap_output_laboratory_values_updated_feb22.csv', row.names = FALSE)
write.csv(flagged, '../local_ref/flagged_labs_for_manual_review_feb22.csv', row.names = FALSE)
```


# Clean the environment and load the latest redcap output generated and the one pushed to redcap. 

```{r}
rm(list=ls())

#load the lastest generated
latest <- read.csv( '../local_ref/redcap_output_laboratory_values_all_updated_feb22.csv') %>%
  mutate( comb = paste( record_id, redcap_repeat_instance, lab_values_visit ))

#load the pushed to redcap
uploaded <- read.csv( '../../laboratory_values/local_ref/redcap_output_laboratory_values_updated.csv') %>%
    mutate( comb = paste( record_id, redcap_repeat_instance, lab_values_visit ))

```

# From the latest extract the new time points
```{r}
# look for diff between dataframes (potential look for new patients and new patients+redcap_repeat_instance combined)
new_data_points <- latest %>%
  filter(! comb %in% uploaded$comb)

write.csv(new_data_points, '../local_ref/new_lab_data_points_to_push.csv', row.names = FALSE)
```

## Extract two patient to test on REDCap
```{r}
not_all_na <- function(x) any(!is.na(x))

test_patients <- new_data_points %>%
  filter(record_id %in% c("120112", '120124')) %>%
  select(-comb) %>%
  select(where(not_all_na))

test_uploaded <- uploaded %>%
  filter(record_id %in% c("120112", '120124')) %>%
  select(-comb)%>%
  select(where(not_all_na)) 

test_patients[is.na(test_patients)] <- ""
test_uploaded[is.na(test_uploaded)] <- ""

write.csv(test_patients, '../local_ref/new_lab_data_points_to_push_test_patients.csv', row.names = FALSE)
write.csv(test_uploaded, '../local_ref/previous_lab_data_points_pushed_test_patients.csv', row.names = FALSE)
```

## Create excel table for data that needs to be overwriten
This includes:
- Values for data points previously entered and that changed the value
- Labs for data points that were empty in the previous pushed and for what we now have a value

We will compare the two final_tables, the previously created and the latest one, and extract the differences, and save it on an table with previous and new values for clinicians to check. 
```{r}
rm(list=ls())
misc_mapping <- read.csv("..//local_ref/music_id_mapping_feb22.csv")
...
previous_patients <- misc_mapping %>%
   filter( MUSIC_ID %in% redcap)

load(file = "../local_ref/finalTableFilteredLatestUpdateData.RData")
load(file = "../local_ref/finalTableFilteredPreviousData.RData")

p1 <- previousDataUpdate %>%
  filter( PATIENT_NUM %in% previous_patients$PAT_NUM) %>%
  mutate( comb = paste( PATIENT_NUM, period, sep = "-")) %>%
  select(PATIENT_NUM, variableName, period, value, comb)
...
combined <- full_join(p1, l1,
                      by = c('PATIENT_NUM', 'variableName', 'period', 'comb'),
                      suffix = c('.prev', '.latest'))

na_prev <- combined %>% 
  filter(is.na(value.prev) &
          comb %in% p1$comb )

combined2 <- combined %>%
  filter( !is.na(value.latest)) %>%
...
#complete the table tocheck  with the music_id
misc_mapping <- read.csv("../../local_ref/music_id_mapping.csv")

tocheck <- tocheck %>%
  left_join( misc_mapping) %>%
  select( MRN, MUSIC_ID, PATIENT_NUM, variableName, period, value.prev, value.latest) 

write.csv( tocheck, file = '../local_ref/lab_values_to_check_manually.csv', row.names = FALSE)

before_na_now_data <- na_prev %>%
  left_join( misc_mapping) %>%
  select( MRN, MUSIC_ID, PATIENT_NUM, variableName, period, value.prev, value.latest) 

write.csv( before_na_now_data, file = '../local_ref/lab_values_to_check_manually_before_na.csv', row.names = FALSE)

...
```{r}
ecrftobch <- read.csv("../data/eCRF to BCH mapping.csv")
...
```{r}
## BCH data dictionary
internalDictionary <- read.delim("../local_ref/lab_bch.dsv")
...
internalDictionary_numericSummary <- internalDictionary_numeric %>%
  ...
  dplyr::summarise( min = min(NVAL_NUM), max = max(NVAL_NUM)) %>%
...
```{r}
# export as file
write.table(final, file = "../data/numericlabsToReview.txt", col.names = TRUE, row.names = FALSE, sep = "\t", quote = FALSE)
```

...
```
Use dates to determine visit number **uncertain about meaning of redcap_repeat_instance and lab_values_visit**
...
finalTable <- finalTable %>% 
  ...
  arrange(date, time) %>%
  mutate(visit = cur_group_id()) %>%
  arrange(visit)

finalTable %>% select(PATIENT_NUM, date, time, visit) 
...
out <- pivot_wider(finalTable, 
                   id_cols = c(PATIENT_NUM, formName, date, time, visit), 
...
out <- out %>%
  ...
  relocate(record_id, redcap_event_name, redcap_repeat_instrument, redcap_repeat_instance, redcap_data_access_group, visit)

...
group_3 <- finalTable %>% 
  group_by(PATIENT_NUM, variableName) %>%
  filter(days_after_discharge == 14) %>% # currently using *exactly* 2 weeks post discharge
  mutate(lab_values_visit = 3)

#4, 6 Weeks Post Discharge
group_4 <- finalTable %>% 
  group_by(PATIENT_NUM, variableName) %>%
  filter(days_after_discharge == 42) %>% # currently using *exactly* 6 weeks post discharge
  mutate(lab_values_visit = 4)

# 6, 6 Months Post Discharge
group_6 <- finalTable %>% 
  group_by(PATIENT_NUM, variableName) %>%
  filter(days_after_discharge == 182) %>% # currently using *exactly* 6 months post discharge
...
combinedFinal <- bind_rows(group_1, group_2, group_3, group_4, group_6, group_23) # currently not including gorup 25
...
out <- pivot_wider(combinedFinal, 
                   id_cols = c(PATIENT_NUM, formName, date, time, lab_values_visit, redcap_repeat_instance), 

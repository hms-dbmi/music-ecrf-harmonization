...
timeline_hosp_summary <- timeline_hosp %>% 
  ...
  dplyr::summarise( n_hosp = mean( length( PATIENT_NUM)), 
                    ...
                    admission_dates = paste(date, collapse=',')) %>%
...
readmission_table <- timeline_hosp %>% 
  ...
  summarise(n_hosp = mean(length( PATIENT_NUM)),
            ...
            END_DATE = END_DATE) %>%
  filter(n_hosp > 1,
         date != dmy(ADMISSIONDATE)) %>%    # remove initial admission dates
  select( PATIENT_NUM, n_hosp, ADMISSIONDATE, date, START_DATE, END_DATE)

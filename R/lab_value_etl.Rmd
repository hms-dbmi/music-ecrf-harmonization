...
timeline$date <- sapply(strsplit( as.character(timeline$START_DATE), " "), '[', 1)
timeline$date <- mdy(as.character(dmy(timeline$date), format = "%m-%d-%Y"))


timeline_hosp <- timeline %>% 
  dplyr::filter( INOUT_CD == "Inpatient", 
                 LENGTH_OF_STAY > 1) 
output <- timeline_hosp %>% 
  dplyr::group_by( PATIENT_NUM ) %>%
  dplyr::summarise( hosp = mean( length( PATIENT_NUM)), 
                    length = mean( LENGTH_OF_STAY)) %>%
  dplyr::select( PATIENT_NUM, hosp, length )
output %>% dplyr::filter( hosp > 1 )

timeline %>% filter(PATIENT_NUM == 'A75C4588-156D-2EC3-E053-DC90240ABE72') %>% 
  ggplot() + 
  geom_point(aes(x = date, y = LENGTH_OF_STAY, group = INOUT_CD, color = INOUT_CD), size = 3) + 
...
# this function defines the *dates*; will still need to go back after and find which are good for each patient (will be multiples)
                                 ...
                                 no = ifelse(test = days_after_discharge >= 0 & days_after_discharge < 21 & abs(days_after_discharge-21) == min(abs(days_after_discharge - 21)),
                                             ...
                                             no = ifelse(test = days_after_discharge >= 21 & days_after_discharge < 63 & abs(days_after_discharge-42) == min(abs(days_after_discharge - 42)),
                                                         ...
                                                         no = ifelse(test = days_after_discharge >= 63 & days_after_discharge < 365 & abs(days_after_discharge-180) == min(abs(days_after_discharge - 180)),
                                                                     ...
                                                                     # readmission will be determined through timeline
                                                                     no = NA)
                                             )
                                 )
                     )

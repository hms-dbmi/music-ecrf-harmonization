...
###########################
setwd("/Users/alba/Desktop/music-ecrf-harmonization/")

###############################
#### Read Sajad Dictionary ####
###############################
datadict_overlap <- read.csv('common_ref/S4j4d_updated_MUSIC_DataDictionary_2020-09-09.csv')

# get the counts based on his auto extraction column 
...
bch_labs <- read.csv("laboratory_values//local_ref/redcap_output_laboratory_values_updated.csv", 
...
#### check data points per time point
bch_labs_timePoint <- bch_labs %>%
  dplyr::select( - redcap_event_name, - redcap_repeat_instrument, - redcap_repeat_instance) %>% # remove columns that are not needed
  tidyr::pivot_longer( cols = c(3:285), 
                       names_to = "variable", 
                       values_to = "value") %>%
  dplyr::filter( value != "") %>%
...
print( paste0( "There are a total of ", 
               nrow( bch_labs_timePoint ), 
               " lab-related data points collected for BCH over time") ) 


bch_labs_timePoint_perTime <- bch_labs_timePoint %>%
  dplyr::group_by( lab_values_visit ) %>%
  dplyr::summarise( totals = sum(dp))

bch_labs_timePoint_perPatient <- bch_labs_timePoint %>%
  dplyr::group_by( record_id ) %>%
  dplyr::summarise( totals = sum(dp))

summary(bch_labs_timePoint_perPatient)

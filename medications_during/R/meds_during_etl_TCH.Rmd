...
fix_code_formatting <- function(x) {
  ...
  if(!is.na(x)) {
    if (!grepl("[[:digit:]]{2}.", x)) {y <- paste0('0', x)} 
    if (!grepl(".[[:digit:]]{2}", y)) {y <- paste0(y, '0')}
  }
...
tch_med_concept_summary <- tch_med_concept_summary %>% rbind(data.frame('keyword' = 'manual', 
  ...
  mutate(ECRF_CODE = sapply(FUN = fix_code_formatting, ECRF_CODE))
...
write dummy id data for test upload to redcap
```{r}
load('../../laboratory_values/local_ref/dummy_ID_mapping.RData')
out8 <- out7 %>% 
...
write.csv(out8, '../local_ref/tch_medications_during_092023_subsetforredcap.csv', row.names = FALSE, na = '')

...
<<<<<<< HEAD
df <- read.csv('../../medications_before_and_after/local_ref/ag440_music_meds_nov21.csv')
=======
...
bch_med_concept_summary <- read_csv('../../medications_before_and_after/data/bch_med_concept_summary_toReview_Tuesday.csv')
...
# remove NAs that appear because as input we are using now all misc and all medications
df <- df %>%
...
=======
  ...
  mutate( time = hms(sapply(strsplit( as.character(START_DATE), " "), '[', 2)),
          datetime = paste0(date, '-', time),
          datetime = ymd_hms(datetime)) %>%
>>>>>>> main
  group_by(PATIENT_NUM, ECRF_CODE) %>%
  summarise(Variable...Field.Name = first(Variable...Field.Name)) %>%
  ungroup() %>%
  group_by(PATIENT_NUM) %>%
  arrange(ECRF_CODE) %>%
...
out <- pivot_wider(df3,
                  id_cols = c(PATIENT_NUM, medhospX_type, redcap_repeat_instance, med_num),   
...
id_mapping <- read.csv('../../medications_before_and_after/local_ref/music_id_mapping.csv')
out4 <- left_join(out3, id_mapping %>% select(PATIENT_NUM, MUSIC_ID)) %>% 
  ungroup() %>% 
  select(-PATIENT_NUM) %>%
...
#test2 <- test %>%
#  group_by(record_id, redcap_repeat_instance) %>%
#  dplyr::summarise_all(~first(na.omit(.)))



#library(data.table)
#setDT(test)[, lapply(.SD, na.omit), by = record_id)]

# write output with patient IDs included
#write.csv(out4, '../local_ref/redcap_output_medications_during.csv', row.names = FALSE, na = '')
...
write.csv(out5 %>%
            filter(record_id %in% c(120006)) %>%
            mutate(record_id = 888006),

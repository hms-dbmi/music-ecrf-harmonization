...
df2 <- df %>% 
  ...
  mutate(medhospX_start_dt = ifelse(grepl(report_dates, ECRF_CODE),
                                    medhospX_start_dt,
                                    ''))
df3 <- df2 %>%
  group_by(PATIENT_NUM, ECRF_CODE, medhospX_start_dt) %>%
...
out <- pivot_wider(df4,
                  id_cols = c(PATIENT_NUM, medhospX_type, redcap_repeat_instance, med_num, medhospX_start_dt),   

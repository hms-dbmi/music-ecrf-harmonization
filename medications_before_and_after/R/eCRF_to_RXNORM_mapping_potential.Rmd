...
```{r}
rxnorm_subset <- read.delim("./rxnorm.dsv")
```
...
```{r}
meds_keywords <- read.csv("./CodeListD(Meds) MUSIC 9-7-20.csv", colClasses = "character" )
...
for( i  in 1:nrow( meds_auto ) ){
  ...
  rxnorm_output <- rxnorm_subset %>% 
    dplyr::filter( grepl( keyword, tolower(STR))) %>%
    dplyr::group_by( CODE ) %>%
    dplyr::summarise( RXNORM_CODE = CODE[1], RXNORM_DESCRIPTION = STR[1]) %>%
    dplyr::mutate( ECRF_CODE = meds_auto[i,"Code"], 
                   ECRF_GENERIC_NAME = meds_auto[i, "Generic.Name"], 
                   RXNORM_CODE = as.character( RXNORM_CODE )) %>%
    dplyr::select( ECRF_CODE, ECRF_GENERIC_NAME, RXNORM_CODE, RXNORM_DESCRIPTION)

  if( i == 1){
    ecrf_to_rxnorm <- rxnorm_output
  }else{

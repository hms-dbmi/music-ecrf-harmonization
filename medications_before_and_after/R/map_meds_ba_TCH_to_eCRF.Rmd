...
Most of the first words, are unique keywords. There are only 7 cases in which we need to take into account the second word to distinguish between different eCRF codes:
...
- Ampicillin vs. Amipicillin/Sulbactam
...
ecrf_to_rxnorm <- read.csv("../data/ecrf_to_rxnorm_mapping.csv") %>%
  dplyr::mutate( ECRF_GENERIC_NAME = gsub("[/]", " ", ECRF_GENERIC_NAME),
                 ...
                 keyword = ifelse( ! first_word %in% c("erythromycin", "isosorbide", "ticlopidine","vitamin", "calcium", "amoxicillin", "amipicillin"), first_word, paste0( first_word, " ",second_word))) %>%
...
tch_meds <- tch_meds %>%
  dplyr::mutate( concept_cd = gsub("[/]", " ", concept_cd),
                 ...
                 keyword = ifelse( ! first_word %in% c("erythromycin", "isosorbide", "ticlopidine",
                                                       "vitamin", "calcium", "amoxicillin", 
                                                       "ampicillin"), first_word, paste0( first_word, " ",second_word)))
...
### replace ampicillin inj by ampicillin
tch_keywords <- tch_keywords %>%
  dplyr::mutate( keyword = gsub("ampicillin inj", "ampicillin", keyword), 
                 keyword = gsub("ampicillin sulbactam", "amipicillin sulbactam", keyword)) %>%
  dplyr::select(concept_cd, keyword ) %>%
  unique()
```
...
```{r}
ecrf_keywords$keyword <- trimws( ecrf_keywords$keyword)
tch_keywords$keyword <- trimws( tch_keywords$keyword)
...
### add some more information to investigate the missing ones
### check which ones that are missing in TCH were present in BCH, and check those
bchMapped <- read.csv("../../medications_before_and_after/data/bch_med_concept_summary_toReview.csv")

potentialMissing <- potentialMissing %>%
  dplyr::mutate( presentInBCH = ifelse( ECRF_CODE %in% bchMapped$ECRF_CODE, "yes", "no"))

write.table(potentialMissing, 
            file = "../../medications_before_and_after/data/tch_missing_med_concepts_toReview.txt", 
            row.names = FALSE, 
            col.names = TRUE, 
            sep = "\t", 
            quote = FALSE

...
rxnorm_meds_subset <- read.delim('../data/rxnorm.dsv')
```
...
# add the RXNORM code and description
# left_join the merged_missing_concepts df with the BCH RXNORM mapping by BCH concept cd
merged_missing_concepts <- left_join(merged_missing_concepts, Medication_mapping, by = c("CONCEPT_CD" = "BCH_CONCEPT_CD"))
rxnorm_meds_subset <- rxnorm_meds_subset %>% mutate(RXNORM_CD = paste0('RXNORM:', CODE))
merged_missing_concepts <- left_join(merged_missing_concepts, rxnorm_meds_subset, by = c("ACT_CONCEPT_CD" = "RXNORM_CD"))

# select columns of interest
merged_missing_concept_summary <- merged_missing_concepts %>% 
  select(Class, Generic.Name, # from eCRF
         ACT_CONCEPT_CD, STR, # from RXNORM
         CONCEPT_CD, BCH_NAME_CHAR, # from BCH
         key, automatic_extraction, has_rxnorm_mapping, rxnorm_in_bch, mapped_but_not_in_bch, BCH_CONCEPT_CD_EXIST) %>% # from us 
  rename(eCRF_Class = Class,
         eCRF_GenericName = Generic.Name,
         RXNORM_CONCEPT_CD = ACT_CONCEPT_CD,
         RXNORM_DESC = STR,
         BCH_CONCEPT_CD = CONCEPT_CD,
         BCH_DESC = BCH_NAME_CHAR)

# define missing use cases
# if RXNORM info is NA, there is a missing mapping
merged_missing_concept_summary <- merged_missing_concept_summary %>%
  mutate(missing_use_case = ifelse(test = is.na(RXNORM_CONCEPT_CD),
                                   yes = "missing BCH to RXNORM mapping",
                                   no = ""))

merged_missing_concept_summary_forreview <- merged_missing_concept_summary %>%
  filter(missing_use_case != 'missing BCH to RXNORM mapping') %>%
  unique()


# merge RXNORM descriptions into one row
merged_missing_concept_summary_forreview <- merged_missing_concept_summary_forreview %>%
  group_by(RXNORM_CONCEPT_CD) %>%
  mutate(RXNORM_DESC = glue::glue_collapse(RXNORM_DESC, sep = ';')) %>%
  unique()

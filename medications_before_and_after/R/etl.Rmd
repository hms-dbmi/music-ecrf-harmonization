...
df <- left_join(agg_music_meds, submapping %>% select(concept_cd, expected.value, Variable...Field.Name), by = c("CONCEPT_CD" = "concept_cd"))

# remove data that we have for the MISC patients which do not map to eCRF variables
df <- df %>% filter(!is.na(Variable...Field.Name))
```
...
# for the lab values, this represented each time a form was filled out
# however, providers are expected to fill a separate form for each medication (?)
# date and time do not matter if the medcur1_current value is the same..?
df2 <- df %>%
...
out <- pivot_wider(df2,
                    id_cols = c(PATIENT_NUM, medcur1_current, redcap_repeat_instance),   

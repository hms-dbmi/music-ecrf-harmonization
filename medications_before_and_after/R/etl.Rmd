...
```{r}
bch_med_concept_summary <- read_csv('../data/bch_med_concept_summary_toReview_Tuesday.csv')
...
df <- read.delim('../local_ref/ag440_music_meds_nov21.dsv')
...
calculate_medcur1_current <- function(days_after_admission, days_after_discharge){
  ...
  else if (days_after_discharge > 0 & days_after_discharge < 14) {return(1)} # 1: Between hospital discharge and 2 Week visit
  else if (days_after_discharge == 14) {return(2)} # 2: 2 Weeks Post-Discharge
  else if (days_after_discharge > 14 & days_after_discharge < 42) {return(3)} # 3: Between the 2 Week and 6 Week visit
  else if (days_after_discharge == 42) {return(4)} # 4: 6 Weeks Post-Discharge
  else if (days_after_discharge > 42 & days_after_discharge < 90) {return(5)} # 5: Between the 6 Week and 3 Month visit
  else if (days_after_discharge == 90) {return(6)} # 6: 3 Months Post-Discharge
  else if (days_after_discharge > 90 & days_after_discharge < 180) {return(7)} # 7: Between the 3 Month and 6 Month visit
  else if (days_after_discharge == 180) {return(8)} # 8: 6 Months Post-Discharge
  else if (days_after_discharge > 180 & days_after_discharge < 365) {return(9)} # 9: Between the 6 Month and 1 Year visit
  else if (days_after_discharge == 365) {return(10)} # 10: 1 Year Post-Discharge
  
  else if (days_after_discharge > 365 & days_after_discharge < 720) {return(11)} # 11: Between the 1 Year and 2 Year visit
  else if (days_after_discharge == 720) {return(12)} # 12: 2 Years Post-Discharge
  else if (days_after_discharge > 720 & days_after_discharge < 1095) {return(13)} # 13: Between the 2 Year and 3 Year visit
  else if (days_after_discharge == 1095) {return(14)} # 14: 3 Years Post-Discharge
  else if (days_after_discharge > 1095 & days_after_discharge < 1460) {return(15)} # 15: Between the 3 Year and 4 Year visit
  else if (days_after_discharge == 1460) {return(16)} # 16: 4 Years Post-Discharge
  else if (days_after_discharge > 1460 & days_after_discharge < 1825) {return(17)} # 17: Between the 4 Year and 5 Year visit
  else if (days_after_discharge == 1825) {return(18)} # 18: 5 Years Post-Discharge
  # 19: Medication given during a readmission to the hospital (only readmissions related to MIS-C/COVID-19 symptoms/complications that occurred within 6 months of the initial hospitalization for MIS-C)
  # 20: At the time of readmission to the hospital (only readmissions related to MIS-C/COVID-19 symptoms/complications that occurred within 6 months of the initial hospitalization for MIS-C)
  
  else if (days_after_admission >= 0 & days_after_discharge <= 0) {return(-88)} # this means that the code happens during hospitalization -- should not be reported here - made up number
  else if (days_after_discharge > 1825) {return(100)} # this means the code happens later than defined in the scope of study -- use a made up number to report
  else {return(999)}
}
df <- df %>%
...
# remove any entries with medcur1_current value of -88 (happened during hospitalization) or 100 (happened after scope of study) or 999 (ERR0R)
df <- df %>% filter(!medcur1_current %in% c(-88, 100, 999))
## add in redcap_repeat_instance 
# "This is a repeating form, with only one medication per form"
df2 <- df %>%
  group_by(PATIENT_NUM) %>%
  select(-START_DATE, -END_DATE, -date, -days_after_admission, -days_after_discharge) %>%
  unique() %>%
...
# *Note the following criteria: "If this is the Week 2 - Month 6 Follow-Up visit, please exclude aspirin."*
# week 2 - month 6 follow ups are characterized by medcur1_current in (3,4,5,6,7)
# aspirin is characterized by ECRF_CODE = 07.02
df2 <- df2 %>%
  filter(!(medcur1_current %in% c(3,4,5,6,7) & ECRF_CODE == '07.02'))
out <- pivot_wider(df2,
...
out <- left_join(out, id_mapping %>% select(PATIENT_NUM, MUSIC_ID)) %>% 
  ungroup() %>% 
  select(-PATIENT_NUM, -medcur1_current) %>%   ### redcap gives the following error: The following fields were not found in the project as real data fields: medcur1_current
...
write.csv(out, '../local_ref/redcap_output_medications_before_and_after.csv', row.names = FALSE, na = '')

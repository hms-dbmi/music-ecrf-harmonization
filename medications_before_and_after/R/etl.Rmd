...
df <- df %>% 
  ...
  mutate(medhosp1_name = ifelse(ECRF_CODE == '99.99',
                                tolower(NAME_CHAR),
                                ''),
         medhosp1_name = ifelse(is.na(medhosp1_name), 
                                'Medication name not recorded in EHR',
                                medhosp1_name))
...
ft4 <- ft3 %>% 
  ...
  select(PATIENT_NUM, ECRF_CODE, medcur1_current, Variable...Field.Name, medhosp1_name) %>%
  unique() %>% 
...
- medhosp1_name: name of medication, only include for 'other' medications



```{r}
out <- pivot_wider(ft5, 
                   id_cols = c(MUSIC_ID, redcap_repeat_instance, medcur1_current, medcur1_current_val, medcur1_type, medhosp1_name),
...
```{r}
write.csv(out4, '../local_ref/redcap_output_medications_before_and_after.csv', row.names = FALSE)
write.csv(out4 %>% 
